...
class KeyManager {
  ...
  process() {
    ...
    while (this.activeConcurrency < MAX_CONCURRENT) {
      ...
      for (let i = 0; i < this.queue.length; i++) {
        ...
        const uid = t.runId + ':' + t.id;
        if (!this.processing.has(uid) && !this.completed.has(uid)) {
          const task = this.queue.splice(i, 1)[0];
          console.log(`[PROCESS] Selected task ${task.id} from position ${i}`);
          this.processing.add(uid);
    ...
    if (!startedAny && this.queue.length && this.activeConcurrency >= MAX_CONCURRENT) {
  ...
  execute(key, task) {
    ...
    try {
      ...
      if (err.code === 'ERR_HTTP2_INVALID_SESSION') {
        ...
        this.processing.delete(task.runId + ':' + task.id);
        this.activeConcurrency--;
  ...
  cancelSiblings(taskId, runId) {
    ...
    this.queue = this.queue.filter(task => !(task.id === taskId && task.runId === runId) && !task.isHedge);
